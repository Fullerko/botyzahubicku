...
def _product_image_sources(product, limit=None):
    return [img.get('src') for img in _product_images(product, limit=limit) if img.get('src')]


def _existing_wc_image_refs(product_id):
    result = _request('GET', f'/wp-json/wc/v3/products/{product_id}', timeout=120)
    ...
    for image in data.get('images') or []:
        ...
        if image_id:
            refs.append({'id': image_id})
    return refs, result


def sync_product_images_to_woocommerce(product):
    """Nahraje do WooCommerce všechny obrázky produktu postupně po jednom.

    Neposíláme hlavní obrázek + galerii v jednom velkém requestu, protože WooCommerce
    si každý obrázek stahuje a ukládá do médií. U více obrázků to často skončí
    timeoutem. Tady se obrázky přidávají postupně: hlavní obrázek, potom galerie.
    """
    if not (product.woocommerce_product_id or '').strip():
        return {'ok': False, 'message': 'Produkt ještě nemá WooCommerce product ID.'}

    sources = _product_image_sources(product)
    if not sources:
        return {'ok': True, 'message': 'Produkt nemá žádné obrázky k synchronizaci.'}

    existing_refs, get_result = _existing_wc_image_refs(product.woocommerce_product_id)
    if not get_result.get('ok'):
        # Když GET selže, pořád zkusíme nahrát všechny obrázky od nuly.
        existing_refs = []

    image_refs = list(existing_refs)
    errors = []
    uploaded_count = 0

    for index, src in enumerate(sources, start=1):
        payload = {'images': image_refs + [{'src': src}]}
        result = _request(
            'PUT',
        ...
        # Jedna krátká opakovačka, kdyby WooCommerce zrovna nestihl obrázek stáhnout.
        if not result.get('ok'):
            time.sleep(2)
            result = _request(
                'PUT',
        ...
        if not result.get('ok'):
            errors.append(f'obrázek {index}: {result.get("message", "chyba")}')
            continue
    ...
    if errors:
        return {
            'ok': uploaded_count > 0,
            'message': f'Obrázky: nahráno {uploaded_count}/{len(sources)}, chyby: ' + '; '.join(errors[:3]),
        }

    return {'ok': True, 'message': f'Obrázky byly synchronizované do WooCommerce ({uploaded_count}/{len(sources)}).'}


def _background_image_sync(app, product_id):
    with app.app_context():
        product = Product.query.get(product_id)
        if not product:
            return
        result = sync_product_images_to_woocommerce(product)
        try:
            product.woocommerce_sync_message = (product.woocommerce_sync_message or '') + ' | ' + result.get('message', '')
...
def start_background_image_sync(product):
    """Spustí nahrávání obrázků mimo hlavní kliknutí v adminu.

    Díky tomu vytvoření produktu nečeká na stažení všech obrázků do WordPress médií
    a uživatel nedostane Internal Server Error. Obrázky se v WooCommerce objeví
    za chvíli po uložení produktu.
    """
    if not _product_image_sources(product):
        return False
    try:
        app = current_app._get_current_object()
        thread = threading.Thread(target=_background_image_sync, args=(app, product.id), daemon=True)
        thread.start()
        return True
    except Exception:
        return False
...
def sync_product_to_woocommerce(product):
    ...
    if start_background_image_sync(product):
        return {
            'ok': True,
            'message': 'Produkt, varianty a SKU byly synchronizované do WooCommerce. Obrázky se nahrávají do WooCommerce na pozadí, za chvíli se objeví u produktu.',
            'data': product_data,
            'raw': result.get('raw', ''),
        }

    return {'ok': True, 'message': 'Produkt, varianty a SKU byly synchronizované do WooCommerce.', 'data': product_data, 'raw': result.get('raw', '')}
